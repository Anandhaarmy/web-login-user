...
const PaymentConfirmationForm = () => {
  ...
  const handleSubmit = (event) => {
    ...
    // Validasi sederhana
    if (formData.namaPenyewa && formData.namaPemilikKost && formData.noRekening && formData.buktiPembayaran) {
      ...
      {showConfirmation? (
        <div className="flex flex-col items-center p-6 bg-white rounded shadow-md">
          <div className="flex items-center mb-4">
            <img src="https://i.ibb.co/w0dD31v/money-bag.png" alt="Money Bag" className="w-12 h-12" />
            <span className="text-xl font-bold ml-4">Pembayaran Berhasil</span>
          </div>
          <button className="bg-yellow-500 text-white py-2 px-4 rounded hover:bg-yellow-600" onClick={handleOkClick}>
            Ok
          </button>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="w-full max-w-md bg-white p-6 rounded shadow-md">
          <h1 className="text-2xl font-bold mb-6">Konfirmasi Bukti Pembayaran</h1>
          <div className="mb-4">
          ...
          <div className="mb-4">
            <label className="block text-gray-700 mb-2">Nama Pemilik Kost</label>
            <input
              type="text"
